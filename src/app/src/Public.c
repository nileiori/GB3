...
*/
/*     //fanqinghai 2016.01.14 统一改成一个接口函数
void Public_PlayTTSVoiceAlarmStr(const u8 *StrPtr)
{
     u16 length;
    
     length = strlen((char *)StrPtr);
    
     if(length < MODTTS_SIZE)
     {
       Modem_Api_Tts_Play(0,(u8 *)StrPtr,length);
     }
}*/
/*************************************************************
** 函数名称: Public_PlayTTSVoiceStr
...
** 全局变量: 无
** 调用模块: 无
** 备注    :由于语音播报数据不能同时占用，所以延时播报,解决以前出现时而不播的情况。
*************************************************************/
unsigned char StrBuff[100];
extern u8 DelayAckCommand;
...
*************************************************************/
extern u32 Alarm;
void Play_TTSVoiceStr()
{

     u16 length;
     length = strlen((char *)StrBuff);
    
     if(length < MODTTS_SIZE)
     {

       if(PUBLIC_CHECKBIT(Alarm,ALARM_BIT_IN_OUT_AREA))
       {
           Modem_Api_Tts_Play(0,(u8 *)StrBuff,length);
       }
       else
       {
           Modem_Api_Tts_Play(1,(u8 *)StrBuff,length);
...
unsigned long Public_GetCurTotalMile(void)
{
    ...
    {
        CurrentMile = VDRPulse_GetTotalMile();
    }
    CurrentMile /=10;
    return CurrentMile;
}
...
void Public_SetCarLoadStatus(unsigned char status)
{
    ...
    //SetEvTask(EV_REPORT); dxl,2016.5.24屏蔽,已
...
//    OpenAccount_CheckAndDisposeLockStatus(); dxl,2016.4.20
    #endif
	  Lock2_TrigOpenAccount();//dxl,2016.4.20
}
/*************************************************************
...
void Public_LcdShowTime(unsigned char y,TIME_T stTime)
{
    #ifdef NEW_LCD_DRIVER_GDI
    char buffer[20];
    sprintf(buffer,"%02d-",stTime.year%100);
...
unsigned char Public_CheckIsPhoneServerRunStatus(void)
{
    ...
    if(EepromPram_ReadPram(E2_RUN_FORBID_CALL_SWITCH_ID,&flag))
    {
        if(1==flag)
        {
            if((Gps_ReadSpeed()>5)||(VDRPulse_GetSecondSpeed()>5))
            {
                return 1;//
            }
        }
