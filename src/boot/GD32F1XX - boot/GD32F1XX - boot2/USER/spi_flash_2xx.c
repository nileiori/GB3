...
void sFLASH_LowLevel_Init(void)
{
  ...
  /*!< Enable GPIO clocks */
  RCC_APB2PeriphClockCmd(sFLASH_SPI_SCK_CLK | sFLASH_SPI_MISO_CLK | 
                         sFLASH_SPI_MOSI_CLK | sFLASH_CS_CLK | sFLASH_WP_CLK, ENABLE);
	
	/* Configure I/O for Flash Chip select */
  GPIO_InitStructure.GPIO_Pin = sFLASH_CS_PIN|sFLASH_WP_PIN;
...
ErrorStatus sFLASH_CheckChip(void)
{
	...
	if((VenderID == 0)||(VenderID == 0xffffff))
  {
	...
	else
	{
		...
		strcpy((char *)WriteBuffer,"abcdefghijklmnopqrstuvwxyz0123456789");
		BufferLen = strlen((const char *)WriteBuffer);
		Addr = 0;
		sFLASH_EraseSector(Addr);
		sFLASH_WriteBuffer(WriteBuffer, Addr, BufferLen);
		sFLASH_ReadBuffer(ReadBuffer,Addr, BufferLen);
		if(0 == strcmp((const char *)WriteBuffer,(const char *)ReadBuffer))
		{
