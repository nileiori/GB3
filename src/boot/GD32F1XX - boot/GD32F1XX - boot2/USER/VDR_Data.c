...
ErrorStatus VDRData_Write(u8 Type, u8 *pBuffer, u16 length, u32 Time)
{
	...
	SPI_FLASH_BufferRead(DataBuffer,Address,Data[Type].StepLen);
	...
	SPI_FLASH_BufferWrite(DataBuffer,Address,length+5);
	...
	SPI_FLASH_BufferRead(DataBuffer,Address,length+5);
...
u16 VDRData_Read(u8 *pBuffer,u8 Type,TIME_T StartTime,TIME_T EndTime, u8 block)
{
	...
	{
		flag = 0;
		for(Step=0; Step<Data[Type].SectorStep; Step++)//在该扇区找到一条有效数据，一般为第1条
		{
...
u16 VDRData_ReadList(u8 *pBuffer,u8 Type,u16 ListNum)
{
	u32 Address;
	u16 Sector;
	u16 OffSector;//偏移扇区数,以最老的那条数据为起点
	u16 OffStep;//偏移步数,指最后的那个扇区的偏移步数,它总是小于Data[i].SectorStep
	u16 OffList;//偏移条数
	u16 TotalList;
	u16 i;
	...
	OffList = ListNum - 1;
	OffSector = OffList/DataPacketList[Type];
	OffStep = OffList%DataPacketList[Type];
	Sector = Data[i].OldestSector;
	for(i=0; i<OffSector; i++)
	{
		Sector++;
	...
	Address = Sector*FLASH_ONE_SECTOR_BYTES+OffStep*Data[Type].StepLen;
	SPI_FLASH_BufferRead(DataBuffer,Address,Data[Type].StepLen);
	if(DataBuffer[Data[Type].StepLen-1] == Public_GetSumVerify(DataBuffer,Data[Type].StepLen-1))
	{
...
***************************************************************************/
/*
static void VDRData_EraseAll(void)
{
	VDRData_Erase(VDR_DATA_TYPE_SPEED);//擦除行驶速度记录
	VDRData_Erase(VDR_DATA_TYPE_POSITION);//擦除位置信息记录
	VDRData_Erase(VDR_DATA_TYPE_DOUBT);//擦除事故疑点记录
	VDRData_Erase(VDR_DATA_TYPE_OVER_TIME);//擦除超时驾驶记录
	VDRData_Erase(VDR_DATA_TYPE_DRIVER);//擦除驾驶人身份记录
	VDRData_Erase(VDR_DATA_TYPE_POWER);//擦除外部供电记录
	VDRData_Erase(VDR_DATA_TYPE_PARAMETER);//擦除参数修改记录
	VDRData_Erase(VDR_DATA_TYPE_SPEED_STATUS);//擦除速度状态日志记录
}
*/
/**************************************************************************
...
void VDRData_Erase(u8 Type)
{
	...
	for(i=Data[Type].StartSector; i<Data[Type].EndSector; i++)
	{
