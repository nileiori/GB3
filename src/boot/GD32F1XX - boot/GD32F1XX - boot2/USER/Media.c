...
u8      RecordSendFlag;//dxl,2015.8 去掉extern //录音文件上传标志,1为上传
...
//dxl,2015.8                    LcdClearScreen(); 
//dxl,2015.8                    LcdShowCaptionEx((char *)"TF卡自检失败",2); 
            }
			f_close(&file);
			CurFolder = 1;
			CurFile = 0;
...
FunctionalState Media_UploadTimeTask(void)
{
	...
	u8      Buffer2[5];
        u32     temp;
        u8 channel = 0;//dxl,2015.8 CHANNEL_DATA_1;//为编译通过而添加
		...
		if(FR_OK == f_open(&file,ff_NameConver(MemoryUploadFileName),FA_READ))
		{
			...
			if(0 != (file.fsize-36)%512)
			{
				MemoryUploadSendCountMax++;
                        ...
                        //dxl,2015.8 Subpackage_Media_File_Name_Set((char const *)MemoryUploadFileName);
	...
	if(0 == flag)
	{
		...
		//暂停任务,待上传完以后再开启
		//dxl,2015.8 ClrTimerTask(TIME_CAMERA);
		///ClrTimerTask(TIME_RECORD);
		//暂停心跳包
                //dxl,2015.8 ClrTimerTask(TIME_HEART_BEAT);
	...
	else
	{
		...
		memset(MemoryUploadFileName,0,33);
                RecordSendFlag = 0;
		//开启拍照
		//dxl,2015.8 SetTimerTask(TIME_CAMERA, 1);
                //SetTimerTask(TIME_CAMERA, 1);//dxl
		//SetTimerTask(TIME_RECORD,RECORD_TASK_TIME);//启动设置调度任务
                //开启心跳包
                //dxl,2015.8 SetTimerTask(TIME_HEART_BEAT, SYSTICK_1SECOND);
